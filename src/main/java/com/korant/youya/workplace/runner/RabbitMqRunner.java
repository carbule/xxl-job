...
public class RabbitMqRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        try {
            ...
            Collection<BindingProperties> normalProperties = mqConfigurationProperties.getNormalProperties().values();
            if (!normalProperties.isEmpty()) {
                log.info("读取到普通队列配置");
                for (BindingProperties property : normalProperties) {
...
//                    channel.basicQos(1);
            ...
            Collection<BindingProperties> delayProperties = mqConfigurationProperties.getDelayProperties().values();
            if (!delayProperties.isEmpty()) {
                log.info("读取到延迟队列配置");
                for (BindingProperties property : delayProperties) {
                    String exchangeName = property.getExchangeName();
                    String queueName = property.getQueueName();
                    String routingKey = property.getRoutingKey();
                    String consumerType = property.getConsumerType();
